...
const { generateProfileCard } = require('./card');
...
async function cmd경험치제거(interaction) {
  ...
  const target = interaction.options.getUser('유저');
  db.resetUserXp(interaction.guild.id, target.id);

  const embed = new EmbedBuilder()
    .setColor(MAIN_COLOR)
    .setTitle('🗑️ 경험치 초기화')
    .setDescription(`${target}님의 경험치와 도박경험치를 모두 0으로 초기화했습니다.`);
...
async function handleGachaDraw(interaction) {
  ...
  if (items.length === 0) {
    return interaction.reply({ content: '아직 등록된 가챠 확률이 없습니다. 잠시 후 다시 시도해주세요.', ephemeral: true });
  }

  const user = db.getUser(guildId, interaction.user.id);
  if (user.xp < GACHA_COST_XP) {
    return interaction.reply({
      content: `가챠를 뽑으려면 경험치 **${GACHA_COST_XP.toLocaleString()}**이 필요합니다. (보유: ${user.xp.toLocaleString()} XP)`,
      ephemeral: true,
    });
  }
...
async function cmd음성(interaction) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(`누적 음성 시간: **${formatDuration(user.voice_seconds)}**\n서버 순위: **#${rank}**`)
    .setTimestamp();
...
async function cmd채팅(interaction) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setThumbnail(member.displayAvatarURL({ extension: 'png', size: 256 }))
    .setDescription(`누적 채팅 횟수: **${user.message_count.toLocaleString()}회**\n서버 순위: **#${rank}**`)
    .setTimestamp();
